        ...
        <div className="max-w-4xl mx-auto p-6">
            ...
            <article className="bg-white rounded-lg shadow-lg overflow-hidden">
                ...
                <footer className="bg-gray-50 p-6 border-t">
                    ...
                    {/* Área de comentarios */}
                    <CommentsSection storyId={news.id} />
                </footer>
            </article>
...
const CommentsSection: React.FC<CommentsSectionProps> = ({ storyId }) => {
    ...
    const handleAddComment = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch('/api/comments', {
                ...
                body: JSON.stringify({ storyId, content: commentInput })
            });
            const data = await res.json();
    ...
    return (
        <div className="mt-6 border-t pt-4">
            <h4 className="font-semibold mb-2 text-gray-700 flex items-center"><MessageCircle className="h-4 w-4 mr-2" /> Comentarios</h4>
            ...
            <div className="space-y-3 mb-4">
                {comments.length === 0 && <div className="text-sm text-gray-400">No hay comentarios aún.</div>}
                {comments.map((c: any) => (
                    <div key={c.id} className="bg-gray-50 border rounded p-2">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-xs text-gray-400 ml-2">{c.createdAt ? new Date(c.createdAt).toLocaleDateString('es-ES') : ''}</span>
                        </div>
            ...
            {user ? (
                <form className="flex gap-2" onSubmit={handleAddComment}>
                    ...
                    />
                    <button type="submit" disabled={commentLoading || !commentInput.trim()} className="bg-blue-600 text-white px-4 py-2 rounded">
                        Publicar
                    </button>
                </form>
            ) : (
